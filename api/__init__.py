...
from . import dependencies, utilities, handlers
from . import user_api, scope_api
...
service.add_exception_handler(exceptions.APIException, handlers.handle_api_error)
service.add_exception_handler(sqlalchemy.exc.IntegrityError, handlers.handle_integrity_error)
service.add_exception_handler(
    fastapi.exceptions.RequestValidationError, handlers.handle_request_validation_error
)
...
async def oauth2_token(
    ...
    if form.grant_type == "refresh_token":
        ...
        database.crud.insert_token_set(token_set)
        database.crud.delete_refresh_token(refresh_token)
        return token_set
        ...
        if not utilities.verify_password(
            form.password.get_secret_value(), user.password.get_secret_value()
        ):
            raise exceptions.APIException(
                error_code="WRONG_CREDENTIALS",
                error_name="Wrong Credentials",
        ...
        database.crud.insert_token_set(user, token_set)
        return token_set
    else:
        raise exceptions.APIException(
...
async def oauth2_check_token(
    ...
    if access_token_information is not None and refresh_token_information is None:
        return models.responses.TokenIntrospection(
            ...
            scope=[
                scope.scope_string_value
                for scope in database.crud.get_access_token_scopes(access_token_information)
            ],
        ...
        return models.responses.TokenIntrospection(
            ...
            scope=[
                scope.scope_string_value
                for scope in database.crud.get_access_token_scopes(refresh_token_information)
            ],
            expires_at=refresh_token_information.expires.timestamp(),
            created_at=refresh_token_information.created,
...
async def oauth2_revoke(
    ...
    if access_token_information is not None:
        if access_token_information.owner_id != user.id:
            ...
            database.crud.delete_access_token(access_token_information)
            return fastapi.Response(status_code=HTTPStatus.NO_CONTENT)
    if refresh_token_information is not None:
        if refresh_token_information.owner_id != user.id:
            ...
            database.crud.delete_refresh_token(refresh_token_information)
            return fastapi.Response(status_code=HTTPStatus.NO_CONTENT)
    return fastapi.Response(status_code=HTTPStatus.NO_CONTENT)
